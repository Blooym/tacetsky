...
pub struct CommandRoot {
    ...
    #[arg(
        long = "data-path",
        env = "TACETSKY_DATA_PATH",
        default_value = dirs::config_local_dir().unwrap().join("tacetsky").into_os_string(),
        global = true
    )]
    data_path: PathBuf,
    ...
    #[arg(
        long = "database-url",
        env = "DATABASE_URL",
        default_value = format!("sqlite://{}?mode=rwc", dirs::config_local_dir().unwrap().join("tacetsky").join("db.sqlite3").to_str().unwrap()),
        global = true
    )]
    database_url: String,
}
