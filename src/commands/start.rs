...
use crate::fetcher::WuwaNewsFetcher;
use anyhow::Result;
use chrono::Duration;
use clap::Parser;
use reqwest::Url;
...
pub struct StartCommand {
    ...
    #[clap(
        required = true,
        long = "app-identifier",
        env = "TACETSKY_APP_IDENTIFIER"
    )]
    identifier: String,

    /// The app password to use for authentication.
    #[clap(required = true, long = "app-password", env = "TACETSKY_APP_PASSWORD")]
    password: String,
    ...
    #[clap(
        ...
        env = "TACETSKY_RERUN_INTERVAL_SECONDS"
    )]
    ...
    #[clap(
        ...
        env = "TACETSKY_NEWS_BACKDATE_HOURS"
    )]
    ...
    #[clap(
        ...
        env = "TACETSKY_DISABLE_POST_COMMENTS"
    )]
    disable_post_comments: primitive::bool,

    /// The locale to use when fetching news posts.
    ///
    /// Known valid options are "en", "de", "es", "fr", "kr", "zh-tw" and "jp".
    #[clap(
        default_value = "en",
        long = "news-locale",
        env = "TACETSKY_NEWS_LOCALE",
        value_delimiter = ','
    )]
    news_locale: String,
    ...
    ///
    /// Note: this may not be the same as the news locale value.
    #[clap(
        default_value = "en",
        long = "post-languages",
        env = "TACETSKY_POST_LANGUAGES",
        value_delimiter = ','
    )]
...
impl ExecutableCommand for StartCommand {
    async fn run(self, global_args: GlobalArguments) -> Result<()> {
        ...
        let mut news_fetcher = WuwaNewsFetcher::new(
            self.news_locale,
            &database,
                    ...
                    for post in posts {
                        ...
                        let post_data = {
                            PostData {
                                ...
                                tags: post.content_tag.map(|t| vec![t]),
